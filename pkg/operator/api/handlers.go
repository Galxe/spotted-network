...
func (h *Handler) SendRequest(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		writeRequestError(w, http.StatusMethodNotAllowed, "Method not allowed")
		return
	}

	var params SendRequestParams
	if err := json.NewDecoder(r.Body).Decode(&params); err != nil {
		writeRequestError(w, http.StatusBadRequest, "Invalid request body")
		return
	}

	if err := h.validateRequest(&params, r); err != nil {
		writeRequestError(w, http.StatusBadRequest, err.Error())
		return
	}

	mainnetClient, err := h.chainManager.GetMainnetClient()
	if err != nil {
		writeRequestError(w, http.StatusInternalServerError, fmt.Sprintf("Failed to get mainnet client: %v", err))
		return
	}
	...
	if err != nil {
		writeRequestError(w, http.StatusInternalServerError, fmt.Sprintf("Failed to get current epoch: %v", err))
		return
	}
	...
	if params.ChainID == 1 {
		...
		if err != nil {
			writeRequestError(w, http.StatusBadRequest, fmt.Sprintf("Failed to get state client for chain %d: %v", params.ChainID, err))
			return
		}
	}
	if stateClient == nil {
		log.Printf("[API] State client is nil for chain %d", params.ChainID)
		writeRequestError(w, http.StatusInternalServerError, "Internal server error")
		return
	}
	...
	if params.Timestamp != 0 {
		...
		if err != nil {
			writeRequestError(w, http.StatusInternalServerError, fmt.Sprintf("Failed to convert timestamp to block number: %v", err))
			return
		}
	} else {
	...
	if err != nil {
		writeRequestError(w, http.StatusInternalServerError, fmt.Sprintf("Failed to get state at block: %v", err))
		return
	}
	...
	if err != nil {
		writeRequestError(w, http.StatusInternalServerError, fmt.Sprintf("Failed to get latest block: %v", err))
		return
	}
	...
	if err != nil {
		writeRequestError(w, http.StatusInternalServerError, fmt.Sprintf("Failed to create task: %v", err))
		return
	}
...
func (h *Handler) GetTaskConsensusByTaskID(w http.ResponseWriter, r *http.Request) {
	...
	if len(taskID) != 64 || !regexp.MustCompile("^[0-9a-f]{64}$").MatchString(taskID) {
		writeConsensusError(w, http.StatusBadRequest, "Invalid task ID format")
		return
	}

	consensus, err := h.consensusResponseRepo.GetConsensusResponseByTaskId(r.Context(), taskID)
	if err != nil {
		writeConsensusError(w, http.StatusInternalServerError, fmt.Sprintf("Failed to get consensus: %v", err))
		return
	}
	if consensus == nil {
		writeConsensusError(w, http.StatusNotFound, "Consensus not found")
		return
	}
...
func (h *Handler) GetConsensusResponseByRequest(w http.ResponseWriter, r *http.Request) {
	...
	if !ethcommon.IsHexAddress(targetAddress) {
		writeConsensusError(w, http.StatusBadRequest, "Invalid target address")
		return
	}
	...
	if err != nil {
		writeConsensusError(w, http.StatusBadRequest, "Invalid chain_id")
		return
	}
	...
	if err != nil {
		writeConsensusError(w, http.StatusBadRequest, "Invalid block_number")
		return
	}
	...
	if keyBig == nil {
		writeConsensusError(w, http.StatusBadRequest, "Invalid key format")
		return
	}
